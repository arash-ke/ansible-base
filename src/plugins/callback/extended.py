...
DOCUMENTATION = '''
    name: extended
    type: stdout
    short_description: extended Ansible screen output
    version_added: historical
    description:
        - This is the extended output callback for ansible-playbook.
...
class CallbackModule(CallbackBase):
    ...
    '''
    This is the extended callback interface, which simply prints messages
    to stdout when new callback events are received.
    '''

    CALLBACK_VERSION = 2.0
    CALLBACK_TYPE = 'stdout'
    CALLBACK_NAME = 'extended'
    ...
    def set_options(self, task_keys=None, var_options=None, direct=None):
        ...
        # for backwards compat with plugins subclassing extended, fallback to constants
    ...
    def v2_playbook_on_include(self, included_file):
        msg = 'included: %s for %s' % (os.path.basename(included_file._filename), ", ".join([h.name for h in included_file._hosts]))
    ...
    def v2_playbook_on_stats(self, stats):
        ...
        if stats.custom and self.show_custom_stats:
            ...
            for k in sorted(stats.custom.keys()):
                ...
                self._display.display('\t%s: %s' % (k, self._dump_results(stats.custom[k], indent=2)), color=C.COLOR_VERBOSE)

            # print per run custom stats
            if '_run' in stats.custom:
                self._display.display("", screen_only=True)
                # self._display.display('\tRUN: %s' % self._dump_results(stats.custom['_run'], indent=1).replace('\n', ''))
                self._display.display('[RUN] => %s' % self._dump_results(stats.custom['_run'], indent=2), color=C.COLOR_VERBOSE)
            self._display.display("", color=C.COLOR_VERBOSE, screen_only=True)
    ...
    def v2_playbook_on_notify(self, handler, host, task=None):
        if self._display.verbosity > 1:
            self._display.display("NOTIFIED HANDLER %s for %s (%s)" % (handler.get_name(), host, task.get_name()), color=C.COLOR_VERBOSE, screen_only=True)