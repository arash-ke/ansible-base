...
def template_print(item, format, delimiter=": ", *args, **kwargs):
    dl = DataLoader()
    templar = Templar(loader=dl, variables=item)
    nargs = []
    nkwargs = {}
    for a in args:
        nargs.append(templar.template(a))
    for k,v in kwargs.items():
        nkwargs[k] = templar.template(v)
    return to_text(format.format(*args, delimiter=delimiter, *nargs, **nkwargs))
